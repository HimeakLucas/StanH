#!/bin/bash
# Waits for the v7 decoder sweep to finish, then evaluates on X-ray + Kodak and
# runs the BD-Rate analysis + plot (compares decoder+quant adaptation vs generic).
export PYTHONPATH=src
export PATH="$HOME/miniconda3/envs/stanh/bin:$PATH"

TAG="v7"
SWEEP_LOG="train/logs/v7_resilient.log"
MODELS_DIR="models/xray_decoder_finetuning_v7"
XRAY_JSON="results/${TAG}_decoder_on_xray_rd.json"
KODAK_JSON="results/${TAG}_decoder_on_kodak_rd.json"
EVAL_LOG="train/logs/v7_eval.log"
LIMIT=24
...
python -u plots/analyze_finetuned.py --tag "$TAG" --label "STanH decoder+quant fine-tune (X-ray)" \
